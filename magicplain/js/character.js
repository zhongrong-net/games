...
Character.prototype.putBomb = function(){
    if(this.bomb_count >= this.max_bombs){ // 检查炸弹数量是否达到上限
        console.log("Cannot place more than " + this.max_bombs + " bombs!");
        return false;
    }
    if(panel.canPutBomb(this.mapx, this.mapy)){
        ...
        panel.putBomb(bomb);
        this.bomb_count++; // 增加炸弹计数器
        return true;
    }
    return false;
}

Character.prototype.eatBombItem = function() {
    this.max_bombs += 1; // 吃到炸弹道具后增加最大炸弹数量
    console.log("Eaten a bomb item! Max bombs increased to " + this.max_bombs);
}
...
Character.prototype.onDead = function(){
    ...
    game.time.events.add(1000, this.destroyEmitter, this);    
}
