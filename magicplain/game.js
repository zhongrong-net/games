...
var loadState = {
    ...
    create: function(){
        ...
        var dianj= document.getElementById('dianj');
...
var playState = {
    ...
    level: null,
    battle_music: null,
    propSpawnTimer: null, // 用于存储道具生成定时器
    ...
    create: function(){
        ...
        console.log('panel.player.health',panel.player.health);
        console.log('isxue',isxue);
        
        
        if(isxue){
            panel.player.health = 200
        }
       
        // 添加道具生成定时器，每5秒生成一个道具
        this.propSpawnTimer = game.time.events.loop(Phaser.Timer.SECOND * 5, this.spawnRandomProp, this);
    ...
    // 新增道具生成方法
    spawnRandomProp: function() {
        // 确保游戏状态是进行中
        if (panel.status !== 'playing') return;
        ...
        // 固定生成血瓶（health_bottle）
        var propType = 'health_bottle';
        
        // 在选定的位置添加道具到props_layer
        panel.props_layer.map.properties= {type: "health_bottle"}

        var tile = panel.map.getTile(position.x, position.y, panel.props_layer);
     
       
        
        if (!tile) {
            // 获取血瓶对应的图块索引（假设图集中血瓶的索引为某个值）
            var propTileIndex = getPropTileIndex(propType);
            console.log('tile',propTileIndex);
            if (propTileIndex !== undefined) {
                panel.map.putTile(panel.map.tilesets[0].firstgid + propTileIndex, 
                                 position.x, position.y, panel.props_layer);
                    //    console.log('放血瓶', panel);
        ...
        // 辅助函数：根据道具类型获取对应的图块索引
        function getPropTileIndex(type) {
            // 这里需要根据实际图集中的位置返回对应的索引
            // 假设图集中血瓶的索引为16（根据实际图集调整）
            var propIndexes = {
                'health_bottle': 20, // 假设血瓶对应的图块索引是16
                // 其他道具...
            };
            return propIndexes[type];
        }
    },
...
function showGameOverModal() {
 ...
 console.log('111111');
 // 重新加载当前关卡
 console.log('playState.map, playState.level',playState.map, playState.level);
 
 game.state.start('play', true, false, playState.map, playState.level);
    ...
    if (videoPlayer && restartButton) {
        videoPlayer.addEventListener('timeupdate', function() {
            ...
            if (restartButton) {
                restartButton.textContent = '重新挑战' + remainingTime + 's';
                if (remainingTime <= 0) {
    ...
    if (videoPlayer && restartButton) {
        videoPlayer.addEventListener('ended', function() {
            if (restartButton) {
                restartButton.disabled = false;
                restartButton.textContent = '重新挑战 0s';
            }
        });
    }
...
function Character(data, panel, mapx, mapy, type){
  ...
  // 血条尺寸和颜色
  this.healthBarWidth = 40;   // 血条宽度
  this.healthBarHeight = 5;   // 血条高度
  this.maxHealth = data.health;  // 最大生命值，用于计算比例

  // 创建血条背景（灰色底）
  this.healthBarBg = game.add.graphics(0, 0);
  this.healthBarBg.beginFill(0x888888);  // 灰色背景
  this.healthBarBg.drawRect(0, 0, this.healthBarWidth, this.healthBarHeight);
  this.healthBarBg.endFill();
  
  // 创建血条前景（红色部分）
  this.healthBar = game.add.graphics(0, 0);
  this.updateHealthBar();  // 初始化血条长度
  
  // 将血条添加到世界中
  var coord = panel.mapToCoord(mapx, mapy);
  this.healthBarBg.position.set(coord.x, coord.y - 5);
  this.healthBar.position.set(coord.x, coord.y - 5);

  // 添加到游戏世界中
  game.world.add(this.healthBarBg);
  game.world.add(this.healthBar);
...
Character.prototype.updateHealthBar = function() {
    ...
    // 计算当前血量比例
    var percent = Math.max(0, this.health / this.maxHealth);
    var width = this.healthBarWidth * percent;

    // 绘制红色血条
    this.healthBar.beginFill(0xff0000);  // 红色
    this.healthBar.drawRect(0, 0, width, this.healthBarHeight);
...
Character.prototype.update = function(){
    ...
    // ==== 血条位置更新：直接绑定角色位置，不再使用插值 ====
    var offsetX = 10;
    var offsetY = 1; // 血条在头顶偏移量

    this.healthBarBg.x = this.sprite.body.x + offsetX;
    this.healthBarBg.y = this.sprite.body.y + offsetY;
...
Player.prototype.update = function(){
    ...
    if(this.input.spacebar.isDown){
        ...
        if(this.bomb_cd > 0){
          ...
          console.log('玩家炸弹',current_bombs);
...
function Panel(map){  
    ...
    this.checkProp = function(character){
        ...
        //   console.log('character获取',character);
        ...
        if(prop){
            console.log('怪物玩家',current_bombs);
