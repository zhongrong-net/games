var playState = {
	create: function(){  // 从 localStorage 中读取存储的分数
		var storedScore = localStorage.getItem('tetrisScore');
		curScore = storedScore ? parseInt(storedScore) : 0;


		var storedlevel = localStorage.getItem('gmvolevel');
		level = storedlevel? parseInt(storedlevel) : 0;



		
		var lineCountGG = localStorage.getItem('lineCountGG');
		lineCount = lineCountGG? parseInt(lineCountGG) : 0;
	...
	console.log('gameover',gameover);
		...
		if(preGameCountDown){
			...
			console.log('游戏结束',ghostY.toString(),lineCount);

			 // 游戏结束时存储行数
			 localStorage.setItem('lineCountGG',lineCount.toString());
